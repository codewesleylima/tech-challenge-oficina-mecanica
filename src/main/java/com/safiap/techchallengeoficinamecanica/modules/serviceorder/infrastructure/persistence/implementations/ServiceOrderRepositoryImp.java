...
@Repository
public class ServiceOrderRepositoryImp implements ServiceOrderRepository {

    private final JPAServiceOrderRepository jpaServiceOrderRepository;

    public ServiceOrderRepositoryImp(JPAServiceOrderRepository jpaServiceOrderRepository) {
    ...
    public Optional<ServiceOrder> findById(UUID serviceOrderId) {
        return jpaServiceOrderRepository.findById(serviceOrderId).map(ServiceOrderMapper::toDomain);
    }

    @Override
    public List<ServiceOrder> findByCustomerId(UUID customerId) {
        return jpaServiceOrderRepository.findByCustomerId(customerId).stream()
    ...
    public List<ServiceOrder> findByVehicleId(UUID vehicleId) {
        return jpaServiceOrderRepository.findByVehicleId(vehicleId).stream()
                .map(ServiceOrderMapper::toDomain)
                .collect(Collectors.toList());
    }
    ...
    public List<ServiceOrder> findByStatus(ServiceOrderStatus status) {
        return jpaServiceOrderRepository.findByStatus(status).stream()
                .map(ServiceOrderMapper::toDomain)
                .collect(Collectors.toList());
    }
}
